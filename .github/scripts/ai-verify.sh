...
STORAGE LAYOUT - read this before judging anything: the data files are range-packed. Every file under data/works/, data/people/ and data/series/ is a PACK holding many UNRELATED records in its entries map, and the file name is only a range bound - the slug its range starts at - never the identity of anything inside it. data/series/wheel-of-time.json holds every series whose slug falls in its range, which is mostly NOT the Wheel of Time; the same goes for work and person packs. Judge every change by the ENTRY it touches - its entry key and its own name/title/authors fields - and never by the file it sits in. Never flag a record for being thematically unlike its pack file's name or unlike the neighbouring entries in the same file; that is the normal state of a range pack.
...
- License layer: THIS REPOSITORY HOLDS THE CC0 CORE ONLY. Every record (work/recording/person/series) must be CC0-1.0; flag any record on any other license. The CC BY-SA community layer (characters and recaps) moved to the separate repository KodeStar/audiosilo-meta-community, so a diff here that touches ANY path under data/works-community/, or that adds a record carrying CC-BY-SA-4.0, is itself a finding to flag - that content does not belong in this repository and CI refuses the tree.
