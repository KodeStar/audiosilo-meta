...
internal/ghhost     the GitHub host allowlist (`Allowed`) and the per-hop redirect policy (`CheckRedirect`) its two callers apply before sending a request to a URL that arrived as DATA - a LEAF, shared by internal/serve's release-asset downloader and internal/issueform's attachment fetcher, each keeping its own extra hosts (serve: api.github.com and, in tests, its httptest origin)
...
also HOST-ALLOWLISTED before the request is built (`checkAssetTarget` over
internal/ghhost: github.com, api.github.com, `*.githubusercontent.com` over https,
plus the configured API base's own origin, which is how the tests point the client
at an httptest server) - the URLs come out of the release JSON and `get` attaches
this server's token to whatever host they name, so a release naming another host
would otherwise be handed the credential. The same rule runs on EVERY REDIRECT HOP
(the client's CheckRedirect), because a `browser_download_url` always 302s to a
CDN host: checked once, the allowlist is one 302 away from being walked past.
