...
`check-latest` on every `setup-go` (go.mod's `go 1.25.0` is a FLOOR, not the
toolchain), which steps `changes` gates and why it is a step-level `if` rather
than a `paths:` filter - are all in the `.github/` entry of the package layout
below. CI
