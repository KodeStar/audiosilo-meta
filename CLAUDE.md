...
`.gitattributes` line keeping the pack merge driver off it. And it is not a
...
inside its work, so it travels with the work slug.
...
the mechanism existed), not a 500. **Authoring the expressive
...
heal the id it stored rather than only learn it is gone (`Server.redirected` +
`snapshot.redirectTarget`, gated on schema_version >= 5, one indexed point query
on the miss path only, and a lookup error degrades to "no redirect" instead of a
500). The request's query string travels with the location (a `?limit`/`?offset`
window describes the request, not the id) and the path is composed from the same
namespace the lookup used, so a redirect cannot point into another family's
route. The `chapters` route is the one that consults it on a 200 path: an unknown
pair is an empty list there, and only a slug the catalogue does not hold can be a
