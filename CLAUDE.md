...
go run ./cmd/metaaudit -data data --profile core -o audit-report  # read-only data-quality audit (~30s)
go run ./cmd/metarepair -data data --profile core --op merge-works --limit 50   # DRY RUN of the repair (~30s)
```
...
the community repo. The flag DEFAULTS to `all` on every tool that takes it and
that is deliberate - "a data root" with nothing said must keep meaning the whole
database, which is what the fixtures and most tests are - so the four passes that
read or write THIS tree say `core` out loud: CI's metacheck/metafmt, the intake
bot's re-render, and the audit/repair pair (which must be told the same thing,
since metarepair re-runs metaaudit's detectors before applying anything). Over
this tree the two readings agree - works-community is simply absent either way -
so the flag states what the root is rather than changing what a pass does; the
teeth are the file ACCOUNTING, which turns a reappearing `data/works-community/`
here into a red check instead of a silently re-adopted family.
...
cmd/metaaudit       thin CLI: the READ-ONLY data-quality audit -> a report directory (`-data data --profile core -o <dir>`; flag wiring only, logic in internal/audit over the internal/titlerule primitives)
cmd/metarepair      thin CLI: applies the audit's NON-ADVISORY proposals to data/ (dry-run by default; `--op`/`--subclass`/`--only`/`--limit` select the wave, `-report <dir>` narrows it to a reviewed worklist, `--write` applies, `-o <dir>` writes the apply-report; flag wiring only, logic in internal/repair). It and metaaudit both take `--profile` (default `all`, like metacheck's and metafmt's) and MUST be given the same one: metarepair re-runs metaaudit's detectors in process, so a report taken under one reading of the tree and applied under another is the drift the fresh-audit gate exists to prevent
...
- **Phase 2**: characters and recaps (spoiler-tagged, position-keyed), the CC
  ...
  **schema + metacheck rules, the `metabuild`/`metaserve` wiring, and four
  ...
  [AUTHORING.md](AUTHORING.md)). **The DATA then moved out** (2026-08-21, the
