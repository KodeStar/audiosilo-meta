...
- **description** - the `description` member of the same works-community entry,
  ...
  like every other own-words field.
...
pkg/extract         PUBLIC epub split (container/OPF/spine/toc -> plain text) + the word-shingle overlap check. `expressiveFields` is the SOURCE OF TRUTH for which sidecar fields the ngram check scans, keyed by the works-community MEMBER NAME (which is also its schema file's stem); a kind with no top-level array - `description` is one flat document - declares its top-level prose instead, and is recognized by it. TestCheckedFieldsMatchSchemas walks the embedded schemas and fails on a capped own-words field that is not listed, so a member kind added to the model cannot silently pass the no-verbatim gate as text nobody looked at
...
`displayDescription`'s - the community's own-words paragraph where the work
carries one, the CC0 record's own field otherwise - which is the one read
`workForABS` makes into the sidecar layer, and the same helper the HTML work page
reads, so the two surfaces cannot disagree about what a work's description is. It never writes: all data is
