...
once redirects are added regularly). The other it used to record is CLOSED:
**no minter creates a record at a retired slug** (`internal/importer/tombstone.go`
states the rule, `internal/issueform` applies the same one), because a tombstone
is a recorded human decision that the retired slug names the survivor. A PERSON
slug is identity, so a retired one resolves to its survivor; a SERIES name whose
BASE slug is retired joins the survivor with no name comparison, while a retired
NUMBERED candidate is occupied and stepped past (its old name is not recorded,
and a wrong join is worse than a new series) - through one walker,
`seriesChain`, so getOrCreateSeries, findSeries and libex-select's
seriesIndex.find still agree and "the first FREE slug ends the walk" counts a
tombstone as taken; a retired WORK candidate is judged AS its survivor by the
identity rules a live record there would get (merge, else step past). Every ride
is one aggregated `Summary.Notes` line. Before this, one retired series name
failed a whole user library import on metacheck's live-source rule (#2320).
