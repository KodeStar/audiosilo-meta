...
cmd/metaaudit       thin CLI: the READ-ONLY data-quality audit -> a report directory (`-data data -o <dir>`; flag wiring only, logic in internal/audit)
...
internal/audit      metaaudit's business logic: the READ-ONLY, deterministic data-quality audit. Ten detector classes over one pkg/check load - near-duplicate works (W-DUP, keyed on the cleaned title + author set, with the series-embedded-in-the-title derivation that finds a retailer's second record of one book; never across languages, and a cluster whose titles state DIFFERENT volume numbers is filed as volume-conflict with no merge proposed), decorated titles (W-TITLE, title-only - a slug is identity and is never proposed for change), unmodeled series memberships (W-NOSERIES), per-series integrity (S-INTEGRITY), duplicate and parenthetical-decorated series (SER-DUP / SER-PAREN, the latter proposing nothing because a parenthetical is often a deliberate second ordering), possible duplicate people (P-DUP, advisory throughout, grouping by the IMPORTER's own initials key via importer.MarkedNameKey so the two can never disagree), sidecar hazards (REF-SIDECAR, a spoiler-gated sidecar on one half of a duplicate pair), field hygiene (F-HYGIENE), and pkg/check's own problems/advisories carried through (LOADER). Read-only BY CONSTRUCTION: it reaches the tree only through check.Load and never opens a pack.Store, so there is no write path to disable. Deterministic BY CONSTRUCTION: every list sorted, every group iterated in key order, no timestamps in the output - two runs over one tree are byte-identical. Writes one NDJSON file per class plus SUMMARY.md; the per-work title derivation is memoized and the series-name lookup is a two-word-bucketed prefix probe, which is what keeps a 280k-work run near half a minute. match.go is a documented COPY of audiosilo-server's pkg/match (the behaviour it needs postdates v1.9.0); title.go layers the audit's own wider vocabulary over it rather than editing it, so the copy stays diffable
