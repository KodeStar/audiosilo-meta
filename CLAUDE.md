...
internal/serve      the API server: snapshot loader, JSON handlers, FTS search, the ABS provider endpoint, GitHub-release poller/hot-swap, and the server-rendered HTML ENTITY PAGES (html.go/shell.go/jsonld.go: /works|people|series/{id} injected into the built Astro shells)
...
beside the legacy `?id=` URL, the data-tree path and a bare slug. Business
