...
//     agreement (placesMatch here, seriesClaim.places there), read under
//     titlerule.WriterPolicy on both sides;
...
type titleContext struct {
	...
	// statesVolume: the title itself says which volume it is (titlerule.WriterPolicy,
...
func (c *composer) titleContextFor(title, formSeries string) titleContext {
	...
	ctx.volume, ctx.statesVolume = titlerule.WriterPolicy.Volume(title, ctx.series)
