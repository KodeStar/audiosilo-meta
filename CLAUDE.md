...
go run ./cmd/metarepair -data data --op merge-works --limit 50   # DRY RUN of the repair (~30s)
```
...
- `metacheck` counts the collisions still in the tree as an ADVISORY class
  (`normalized-duplicate-works`, 2,881 groups when it landed), so a repair wave's
...
cmd/metarepair      thin CLI: applies the audit's NON-ADVISORY proposals to data/ (dry-run by default; `--op`/`--subclass`/`--only`/`--limit` select the wave, `-report <dir>` narrows it to a reviewed worklist, `--write` applies, `-o <dir>` writes the apply-report; flag wiring only, logic in internal/repair)
