...
go run ./cmd/metadiff --base <sha> --head <sha>    # entry-level summary of a data change (what ai-verify reads)
...
(`.github/workflows/check.yml`) gates THE WHOLE of it on every PR and push to
main, in five jobs: `changes` (which of the others the diff can move),
`check` (build, vet, `go test -race`, metacheck, metafmt), `lint`
...
`go-version-file: .go-version` + `check-latest: true` pair on every `setup-go`
(go.mod's `go` line is a FLOOR, not the toolchain), which steps `changes` gates and why
it is a step-level `if` rather than a `paths:` filter - are all in the
`.github/` entry of the package layout below. CI
additionally runs a **compose** job - the pre-merge half of the
...
can't both base a patch on the same prev tag. Go 1.26; golangci-lint v2 at a
...
author-suffixed candidate (`primaryWorkCandidates` marks the reserved base probe-only,
...
else that touches the table. One gap is open by design and recorded on
...
once redirects are added regularly). **No minter creates a record at a retired
slug**: a retired person slug resolves to its survivor, a retired series BASE
slug joins its survivor (a retired numbered candidate is stepped past), and a
retired work candidate is judged as its survivor - one lookup,
`model.Redirects.Survivor`; the per-family rule is stated once, in
...
pkg/redirects       PUBLIC slug TOMBSTONE table (data/redirects.json): Load/Parse/Write/Add over model.Redirects. Parse is Load's second half exported for a reader that already HOLDS the bytes and cannot use the working directory (internal/recorddiff reads the file out of a git revision), so the duplicate-key refusal, the decode and the namespace normalization stay one implementation rather than being spelled a second time wherever the file is read from. Add is the whole reason it is a package rather than a map assignment - it collapses chains in BOTH directions (a target since retired, and the tombstones already pointing at the source), so every entry names a live record directly and a resolver does one lookup; it refuses what it cannot decide (an unknown namespace, a non-slug, a reserved route literal, retargeting an existing tombstone, a target that collapses back onto the source) and is idempotent, so a repair pass can be re-run. It knows the FILE, not the database: whether a source is really retired and a target really exists are pkg/check's rules over the tree the writer produced. Its ONE caller is internal/repair, the duplicate-merge pass this mechanism was built ahead of: every slug a merge retires is tombstoned in the same change, and the chain collapse is what keeps the table one hop deep as waves land over each other. internal/remediate's GraphicAudio fold predates it and was not retrofitted (a one-off that has already run)
pkg/extract         PUBLIC epub split (container/OPF/spine/toc -> plain text) + the word-shingle overlap check. Every archive member it reads goes through ONE `readZipFile`, BOUNDED at `maxEntryBytes` (64 MiB decompressed, several times the largest real member - a whole novel in one spine document is under 10MB of XHTML) through an `io.LimitReader` (a member whose header already declares more is refused before it is opened; the header is the attacker's to state, but archive/zip fails any read that runs past it, so an understated size gets a zip error rather than the bytes; that enforcement is unconditional, so no archive/zip input reaches the LimitReader and it is defence in depth): a deflate stream expands ~1000x, so a few-megabyte crafted epub otherwise exhausts memory. Split additionally caps the spine's decompressed TOTAL at `maxBookBytes` (256 MiB), because a spine may list one member many times - a warning, not an error - and each listing is read and written again; it is summed from the DECLARED sizes before anything is written (sound for the reason above), so a refusal leaves the output directory untouched. Either cap refuses the whole epub with `errEntryTooLarge` - the toc included, which is otherwise allowed to be unreadable, since an oversized one is a hostile archive rather than a missing toc; `TestReadZipFileStopsAtTheCap` pins the bound by what a refusal ALLOCATES (for an honest and an understated header; it reads process-wide MemStats, so it never runs in parallel), because a length check alone would still refuse after reading everything. `expressiveFields` is the SOURCE OF TRUTH for which sidecar fields the ngram check scans, keyed by the works-community MEMBER NAME (which is also its schema file's stem, and the key of its per-item array where it has one - a kind with no `itemField`, `description`, is one flat document and contributes only its top-level prose). A BARE record is discriminated by that kind's SCHEMA-REQUIRED keys, never by one suggestive key: a whisper transcript is `{"text": [...]}`, and under a one-key rule that read as an empty description - zero findings, gate passed, prose nobody scanned - which is the silence collectExprs promises never to produce. Inside a PACK the member's KEY names the kind and nothing is re-discriminated at all; either way a declared field present under the wrong JSON type is a loud error rather than a skip. Three drift guards: TestCheckedFieldsMatchSchemas (a capped own-words field the schemas carry must be listed, so a member kind cannot silently pass the no-verbatim gate as text nobody looked at), TestRequiredKeysMatchSchemas (the discriminator IS the schema's own `required`) and TestSidecarDiscriminatorsAreDisjoint (no record can read as two kinds)
...
internal/importer   OpenAudible books.json + Libation export + libex rows -> work/recording/person/series, ASIN-dedup, writes through the pack.Store under `Options.Profile` (the root's TREE PROFILE; zero value ProfileAll, so every existing caller is byte-identical, and it reaches both the store open and the post-write validation - a SCOPED root must never be validated as ProfileAll, which is the one reading under which a leftover family in the wrong repository looks clean. It never narrows what a run WRITES: writeFamilies is still the importer's own answer to that. The two standalone libex modes - libex-select's loadSeriesIndex and libex-fill's LoadCatalogForFill - take a bare dataDir from a CLI with no --profile flag, so there is no profile to thread there and they stay ProfileAll by the same default rule; adding that flag means threading it) (write.go: one works-family composite entry per work, so a recording edit is a read-modify-write of its work's entry; nothing reaches disk until flush; a legacy-layout tree is refused before anything is planned; a create at a slug the store already holds fails loudly rather than overwriting - the guard against a loader-dropped entry being clobbered). added_at is stamped ONLY on the branches that create a record, never on an ASIN merge or enrichment backfill. No chain ever CLAIMS a reserved slug (the API route literals): the work chain marks the reserved base probe-only, the series chain shifts by one (SeriesSlugAt), and person ids come from model.PersonSlug, which composes the reserved variant itself. Nor does any chain claim a RETIRED slug (`tombstone.go`, which states the rule). **Series resolution is AUTHOR-AWARE** (`seriesauthors.go` holds the fit and its measurement, `seriesresolve.go` the resolution): a same-named series is joined only when it FITS the row. The fit has three values. SHARED: a row author is a member's author, by slug or a spelling of the name (`personForm.same`: the marked initials key, the fold, one edit on a folded name of at least eight letters (`MinPersonEditLen`), an inserted middle name (`MiddleNameVariant` - both exported because internal/audit's series vetoes read the same two rules), and INITIALS standing for the words they begin, every initial consuming one word in order with the surname fixed ("L. Frank Baum" / "Lyman Frank Baum", never "J.F. Holmes" / "John Holmes" or "SJ Bennett" / "Sophia Bennett")) - and an author is every spelling of one person at once (a union of the credited slugs, a spelling that matches two groups bridging them), so a person whose records forked into two slugs is one dominant author rather than two minorities. Where one author DOMINATES the series (credits three quarters of its members) the shared author must be that one or co-credit a member with them, so a guest who co-wrote one volume cannot open a franchise to their own same-named series. OPEN: fewer than two members crediting an individual (a collective credit is no evidence), no dominant author, the row's title naming one of the series' OWN authors (a dominant one, or one co-credited with it - never an earlier squatter), or a member sharing the row's publisher where that publisher is not a catalogue-wide house (`largeHouses`: at least 100 works and more than 1% of the catalogue's - measured, the refusals that line adds are two thirds squatters, and lowering it further adds refusals only a third of which are; the 100-work floor keeps a small press in a small catalogue a small press). CLOSED otherwise, and a closed series is stepped past like a differently-named holder, so an unrelated author's "Lost Fleet" joins or mints `lost-fleet-2` instead of squatting another author's slots. Evidence counts WORKS, not rows (`nameClaim.work`, `planner.rowWorkKey`: the catalogued work the row merges into when the create path's own `resolveWork` finds one, else the row's cleaned work title and identity authors, so a title's per-region sibling rows and its edition-marked spelling are one member), and a claim that will not be written is none at all (`claimPlaces` asks addBook's admission tests - an ASIN the catalogue holds, an unknown language, no narrator, no author, no title - before anything is planned, and a claim with no usable position is not counted). Resolution is a BATCH SNAPSHOT (`resolveSeriesClaims`, the initials census's discipline): every claim is decided before anything is placed, against the catalogue plus the batch's own rows, so no outcome depends on row order - SHARED candidates are joined first, in rounds each judged against the evidence as the round began; the rest are clustered by shared author and each cluster, largest first, joins the first catalogued candidate or batch-founded series that ADMITS it (`seriesAuthors.admits`: open to the cluster, and no person dominating the merged evidence who did not already own the series - a one-volume series is open to any one row but not to six rows of another author at once, and an earlier squatter's volume does not become the majority by a batch of the squatter's rows), else founds the next free chain slug (`TestSeriesResolutionIsOrderIndependent`, `TestABatchDoesNotTakeOverAnOpenSeries`). A founded series none of whose rows survives planning (the duplicate-identity guard reads the resolved claim, so its drops cannot be foreseen) is never written, and `settleNewSeriesSlugs` re-mints the written ones through `mintSlug` in founding order (the chain index the resolution stored on the target) at finalize, so no gap is left for a later run to fill with a second series of the name; each series' slug note is emitted then, once the slug is final. Every caller reads that one decision through one `SeriesAuthorIndex` (one catalogue pass, built only when a batch has a series claim, `SeriesAuthorIndex.catalogue`) and one row builder (`SeriesRowFor`): getOrCreateSeries and every claim/placement/enrich path through `seriesFor`; libex-select through an importer PLANNER loaded over the same catalogue (`seriesIndex.p`), so its claims are the importer's own (`planner.batchClaims`, with the kept batch's credit censuses and initials decision) and a selection cannot count a book or a person differently from the import of it (a row whose candidates are all closed is excluded as `catalogue series belongs to other authors`; the kept rows are re-resolved as a batch and the per-series cap applied, the two to a FIXPOINT, since a cut can remove the row another's evidence rested on; a row the batch sends to ANOTHER catalogued series of the name follows it there and claims its position in it, and only the groups a dropped row claimed into are resolved again); and internal/issueform through `SeriesAuthorIndex.Resolve`. The same-title serial guard keys positions by the RESOLVED target slug (`rowSeriesClaims`), so a retired spelling meets its survivor's, and a row's claim names both its source's and its title's position (`rowPosition`), the title's counting only when the row is the same production as one of the work's recordings (`titleCorroborated`). (Shared pipeline over a typed sourceBook, with three disjoint planning modes over it: create, enrich.go's ASIN-matched backfill, recordings.go's alternate-narration pass; conflicts.go is the optional NDJSON conflict worklist - the durable twin of the contradiction warnings, written where the guard fires and nil unless --conflicts was passed; audiblegenres.json maps Audible genre browse-node ids, category PATHS and names onto the schema's genre enum, drift-guard + golden-anchor tested; OpenAudible's `genre` ladder is read through the path table and its `duration` "H:MM" is the runtime a current export carries, since it writes no `seconds`). Beside the pure helpers its in-module neighbours have long shared (the slug composers `BoundedSlugTail`/`NumberedSlugAt`/`AuthorSuffixedWorkSlug`/`SeriesSlugAt`, `ToSet`/`SameSet`, `YearOf`, the credit-name cleaning), it exports two RULES OF RECORD, each because a second copy would be a contract with two definitions: `MarkedNameKey` (the initials identity the importer merges spellings on, read by internal/audit) the ASIN-merge guard's own same-production pair `RuntimesCompatible`/`AbridgedConflict` (read by internal/repair, which asks the identical question of two recordings colliding on one key: the boundaries are within-10%-OF-THE-LARGER and absent-reads-as-unabridged, and a second spelling of either got both wrong), the SERIES-POSITION primitive `PositionSpan`/`PositionRange`/`SameSlot` (the NormalizeSequence-then-ParsePositionRange two-step, whose ORDER is load-bearing; it lives here beside the rule it composes over because internal/titlerule is a true leaf now, and it replaced the five spellings that two-step had grown across internal/audit and internal/repair), and the long-standing `NormalizeSequence` (position acceptance + canonical spelling, tolerant of the 104 real "1 - 3" range spellings the schema pattern rejects, read by internal/audit and internal/issueform). The edition-marker rule went the other way - it MOVED to internal/titlerule (`HasEditionMarker`/`StripEditionMarkers`, which `cleanWorkTitle` now strips through) when pkg/check and this package both became consumers of the normalized title identity built on it. **dupidentity.go is the CREATE path's duplicate-identity guard**: before a row mints a NEW work its normalized identity (`check.WorkIdentity`, TAKEN off the catalogue load's `Result.Identity` - the one index the intake gate and metacheck's census also read - and only for a create run) is probed against the catalogue AND against the works this run has already written; the key is derived ONCE per row and probed before the row's credits are resolved, so a row whose identity nothing holds costs two map lookups rather than a cleaning pass over its credit names, and a row the create path would CREATE a work for is SKIPPED - "would create" asked of `resolveWork` itself, the one read-only decision both the guard and addBook call (addBook calls it again with the author set it actually created rather than reusing the guard's walk), and both resolve a row's credits through the ONE `resolvePerson` (getOrCreatePerson's decision without its effects, also read by `rowWorkAuthorsRO`, `rowProductionOf` and `workCredits`), so the guard cannot stand aside for a merge the create path does not perform - the reachability test it replaced walked both titles' chains while the create path only retried the full title when a series claim blocked, which is how #2337 created 17 sibling works. `resolveWork` walks the row's own title first and, when that holds no match, the FULL title's chain - as a MERGE target only (`mergeOnlyWalk`: no serial position probes, and only when the bare slug is occupied, `bareSlugOccupied`) unless the series claim blocked the short chain (the long-standing retry, which may create there). A full-title walk is judged by a claim read off the FULL title (`seriesClaim.forTitle`), and the title the row was resolved by is the one the recording's serial guard and the series placement read a stated volume from, so a short-titled export row ("Nightfall", or "Towerbound" at Audible's renumbered position) meets the full-titled record the bulk mirror created for the same production and the ordinary ASIN-merge path decides the recording. On EVERY walk (create, blocked retry, full-title merge, recordings-only) a hit on a candidate the MaxSlugLen cap CUT (`workCandidate.shortened`, set where `model.SlugifyCut`/`boundedSlugTail` really cut) counts only when the work there carries the walked title whole (`titlerule.CompareKeyWhole`, the comparison key without the slug cap): a long retail title loses its volume tail at the cut, so a cut hit with a different title is some other book and the walk steps past it. A skipped row is counted as `Summary.SkippedDuplicateIdentity`, named in one aggregated warning, and written to the `--conflicts` worklist as a `work_identity` row carrying both titles. It skips rather than merging or routing to recordings.go on purpose: a refused row is re-importable and a wrong merge is not, and attaching a recording would assert "this IS that book" on exactly the evidence internal/audit measured five vetoes for. Five vetoes keep it off the shapes a title cannot judge - a row the serial pre-pass suffixed (its siblings collide by construction), a row whose series claim puts the matched work at a DIFFERENT position (`seriesClaim.compatible`, read from the series record), a row whose own TITLE states a volume the catalogue does not positively place the matched work at (`seriesClaim.places`: silence is a veto, not agreement, which is what stopped "Circus of the Dead, Book 2" being refused against the plain "Circus of the Dead" when no series, no membership or no claim could confirm it), a COLLECTION on exactly one side (inherited from `check.WorkIdentity.matches` through `titlerule.SameCollectionStatus` - a boxed set is not the volume it collects), and a key naming SEVERAL works (which cannot say which one the row is, the bare-series-name serial shape). The intake gate applies the last three too, so an intake verdict can never contradict what a re-import would do. Nothing about enrich or recordings-only changes: neither takes the index. **seriespos.go settles WHICH POSITION a claim is placed at**, in two narrow rules measured on the two intakes that prompted them. (1) A MISSING position is filled from the live libex service: a personal library export states the series a file is tagged with and, on those intakes, no part number on 97 of 123 series rows - every one of them a `missing or invalid position` warning and a work left out of its series, over a fact the retailer holds. The lookup is an INTERFACE on `Options.SeriesLookup` (`SeriesPositionLookup`), nil by default, so an offline run and every existing test are byte-identical; `cmd/metaimport` and `cmd/metaissue` pass the real `LibexClient` behind `--series-lookup` (which the intake workflow gives the bot) and a test passes a map. It is USER-LIBRARY CREATE only (the gap belongs to a personal export and only the create path places a work), bounded per run by `SeriesLookupLimit` counting only the rows that NEED a lookup (default 100, libex-fill's `--limit` posture: a free public service), and best-effort throughout - a position is used only when libex names the SAME series by the importer's own test (`sameSeriesName`, the series chain walk's slug-bucket-plus-EqualFold rule, not a looser one) with a position `NormalizeSequence` accepts, and anything else leaves today's warning exactly as it was. One aggregated note per run says what was filled, and a second says what could not be reached, because a run whose lookups all failed and a run that needed none otherwise land identical records. (2) A position the row's own TITLE contradicts is arbitrated in the TITLE's favour: "Towerbound, Book 6" arrived at `series_position` 8 - Audible's index counts the side stories this catalogue numbers 3.1 and 5.1 - and was placed at 8 until a maintainer moved it. The title is read under `titlerule.WriterPolicy` against the series name the row states, the SAME call on the same two strings dupidentity.go's positive volume test makes, so the two readings of one title cannot disagree; it additionally requires a volume MARKER (`titlerule.HasVolumeMarker`), because `bareSeq` also reads a residual that is nothing but digits and a work titled "1984" would otherwise be placed at position 1984. Every collision rule is untouched - a work the series already lists keeps its entry, a title position another work holds is not free to move into so the source's stands, and a taken source position reaches addToSeries's own warning by the path it always did - and a range/omnibus position is never arbitrated. It applies to EVERY source, libex included, because the rule is about a title contradicting its own source. `Summary.Warnings` is in REPORTING order (`planner.result`), three tiers each in the order raised: the run-level (aggregated) lines, then the CONFLICT lines (a row refused for contradicting a recorded runtime or release date - what the intake verdict's "disagreed ... see the warnings below" note points at), then every other per-row line `bookWarn` raised; a libex CREATE run's per-row parse-layer lines follow the planner's row lines rather than interleaving with them (the bounded modes' aggregated parse lines lead, as run-level lines). `Summary.RunLevelWarnings` says where the first tier ends and `RowWarnings()` is the rest, so a reader showing only the head of a long list - the intake verdict's bound - keeps what a maintainer acts on, and one that wants the row evidence (issueform's "may not match the selected export type" examples, `falloutExamples`) can ask for it
internal/issueform  issue-form parse + compose (add-work/recording/correction/characters/recaps/import) -> dedup -> pack entries through the pack.Store (both community sidecars share ONE works-community entry, so placing recaps preserves an existing characters member; family gating is per template; a legacy tree is needs-human, not the contributor's fault; a person-name correction that would change the record's SLUG is a rename - the record moves and every work crediting it has to be rewritten - so it is needs-human too, rather than being written and then reported as "invalid" by the person-slug metacheck rule; a title or series name that slugs to a RESERVED route literal steps onto the same candidate the bulk importer would mint - AuthorSuffixedWorkSlug / SeriesSlugAt / PersonSlug - and says so in the verdict, rather than composing a record metacheck would bounce (`seriesForForm`, over the importer's own author-aware resolution `SeriesAuthorIndex.Resolve`, is the one series lookup placement and the duplicate gates share - a same-named series of other authors is never extended: the form composes the submitting author's own series at the next free chain slug and says so); and no name is composed at a RETIRED slug either, by internal/importer/tombstone.go's rule), the ok/duplicate/needs-human/invalid verdict the intake workflow branches on; Result.Files names the pack files flush rewrote - on the IMPORT template too, where the bulk importer does the writing and reports its own flush as `importer.Summary.Files` (result.json used to report nothing there; the pull request body now lists the tree the bot commits instead - see the .github/ entry - since the libex fill step can widen it). The import template is dispatched BEFORE the composer's catalogue load (`loadExisting`), straight after the store opens (whose legacy-layout refusal is still its verdict): the importer loads the catalogue through a store of its own, so the composer's dedup maps were a SECOND whole-catalogue load held live for the importer's entire run (10.8 -> 7.8 GB peak RSS on the real tree). Its file list lands in the same `wrote` field the compose path's flush fills, so both report through one `fileList`. Its attachment field is labelled "Export file"; the old "Additional notes" heading is still read (`fImportAttachmentLegacy`, through `sections.firstRaw`), because GitHub renders the label an issue was OPENED with and an edit re-runs the bot on it, and `TestLegacyLabelsAreGoneFromTheirTemplates` keeps the legacy label out of the template. **dupidentity.go adds the three gates the EXACT ones cannot answer** (the audit's 4,596 near-duplicate clusters walked past all of ASIN/ISBN, narrator set and work slug, because a decorated title compares exact to nothing): a SERIES-VOLUME gate first (a title stating a known series and a volume number that series already fills is that member - the most specific claim, so its verdict names the volume), then the DECORATION STRIP (`titlerule.StripDecoration` at compose time, which is also what lets the existing slug gate see the collision at all - "Mageling (Unabridged)" cleaned to "Mageling" collides where the decorated spelling did not; a strip whose residual names no book or reads as a fragment is needs-human naming the reason, and every other refusal leaves the title as submitted, since a title that IS its series' name is ordinary), then the NORMALIZED-IDENTITY gate (`check.WorkIdentity.Match`, the same index the bulk guard reads, with the language and author-NESTING rules so a catalogued work whose author list carries a role-credited translator still meets a form that names only the novelist). The two DUPLICATE gates route through `failDuplicateWork` - `failDuplicate`'s sibling for a work record - so the bulk-mirror tier discipline is the existing one at all five duplicate gates, while the strip is not a duplicate verdict at all (it rewrites the title or refuses on its own terms). All three read ONE derivation of the submitted title (`titleContext`: the series it is about, the record we hold for it, and whether it states a volume that record has no work at), so the series resolution happens once per submission. The bounds are the bulk guard's, restated in the intake bot's terms so the two doors cannot disagree: a title that STATES a volume is only a duplicate of a work the catalogue positively places at that volume (`placesMatch`, the mirror of `seriesClaim.places` - silence is a veto), a COLLECTION is not the volume it collects, and a key naming SEVERAL catalogued works decides nothing (the ambiguity veto). The STRIP reads the same rules before it rewrites anything (`sameBookAs`): a title whose cleaned form lands on a work this submission is not a second record of keeps its decoration instead, which is what stops "Hammered, Book 7" and "Hammered: The Complete Boxed Set" being handed the slug gate's duplicate verdict for books we do not hold. A series name the INDEX would not read a title against - one significant word, or a fold another series also spells - is not read against one here either (`WorkIdentity.Admits`), so what a submission composes never depends on how an unrelated series spells its name, and a one-word series' sequel ("Cars 2") keeps the number that is its whole identity. The work-slug gate itself stays author-BLIND as it always was, so a decorated title for a different book that cleans onto a taken slug reaches that verdict rather than being composed (the conservative failure - nothing is written). The import template inherits the bulk guard instead: a run whose only outcome was refused duplicates is needs-human, not `duplicate`. Before that allowlist is asked, a correction is judged against the EMBEDDED SCHEMAS (`schemafields.go`: `recordFields`, each correctable kind's field table from `check.FieldsOf` - read off the COMPILED schemas, so the jsonschema library resolves every `$ref` and there is no second schema reader; the genre vocabulary the add-work form validates against is the same table's), for the two submissions nobody could apply as filed - so "a maintainer will apply it" would be untrue: a field the addressed record lacks but its work/recording SIBLING has (`misaddressedField`: `runtime_min` against a work URL is `invalid` naming the recording reference form and listing the work's recordings; a work field against a recording names the work's page URL; a name the record carries NESTED, a work's `xref.isbn`, is not redirected), and a value outside the schema's closed vocabulary (`enumViolation`: a license other than CC0-1.0, a person kind outside the enum, a genre outside the vocabulary - `invalid` naming the allowed values; an in-vocabulary value is then spelled the schema's way, so "Publisher" is the enum's "publisher"). ANY scalar corrected to the value the record already carries is a no-op duplicate that writes nothing (`unchanged`, compared as JSON after coercion) - not a provenance-only write and, for a field the form cannot write, not a maintainer's job. The recording reference a verdict hands out is `recordingRef`'s shard-less, EXTENSION-less `data/works/<work>/recordings/<rec>`, which `refPath` reads back (`TestRecordingRefRoundTrips`), so no shard rule is spelled in production code - the missing `.json` is load-bearing, since with it a recording slugged `work` (or `characters`/`recaps`/`description`) spells the four-segment shape of a WORK named `recordings`. The suggestion is built from the work the record lives under NOW: a retired slug is followed through the tree's tombstone table (`liveWorkSlug`, the verdict-free half of `resolveWorkKey`) and the message says it was merged; the work reference is the page URL over `model.SiteURL`, the one spelling of the public origin (metaserve's `--site-url` default reads it too). `TestCorrectableFieldsAreSchemaFields` pins every allowlisted field to a top-level property of its kind. Corrections run off ONE op-typed allowlist (`correctableFields`, values are a `correctOp` carrying either a scalar coercion kind or an add func, so a listed field always has an op and can never be reachable with nothing to do - which would stamp provenance onto a record it left unchanged). Most fields are scalars the correction REPLACES; two on a recording are ADDITIVE - `isbn` and `publishers` contribute one entry to an array of independent facts rather than replacing the list - an ISBN nobody recorded is appended, an ISBN recorded BARE and corrected with a region is upgraded in place (a bare entry states no region, so scoping it only adds information), and a DIFFERENT stated region, or a different imprint for a region already listed, is needs-human because a stated fact is never rewritten mechanically. The add forms take the same two shapes (`region: ISBN` lines beside bare ones, `region: Publisher` lines) and refuse the schema's/pkg/check's own violations at compose time - a restated publisher of record, a repeated region, publishers[] with no publisher - so a submitter gets a verdict naming the fix instead of a raw metacheck line - and the same pair is guarded from the SCALAR side too (correcting `publisher` to a name publishers[] already lists for a region is needs-human, not a write that metacheck then bounces). Note the deliberate ASYMMETRY in the two identifier parsers: a bare ASIN defaults to `us` (an ASIN only exists inside a marketplace), a bare ISBN stays region-UNSTATED (that is a true state, and inventing one would be a fabricated fact). **It is PROFILE-AWARE** (`Options.Profile`, threaded to the pack store, to `check.LoadProfile`'s post-write validation and to which TEMPLATES are in scope), which is what lets the COMMUNITY repository run the same bot: a form is admitted iff every family `writeFamilies` says it writes is one the root holds, DERIVED rather than a second list of which forms are core and which are community, so ProfileCommunity admits the two sidecar forms alone (everything else is needs-human naming the core repository's issue chooser) and ProfileCore is its mirror image. The `import` template threads it one level FURTHER, into `importer.Options.Profile` (the store open and the post-write validation), because that is the one form that hands the tree to another writer and a scoped root validated as ProfileAll is the one reading under which a leftover family in the wrong repository looks clean. The WORK-EXISTENCE question is the other half: a sidecar's entry key IS a core work slug, and a community root cannot see the works family, so `worksdb.go` asks the newest data release artifact instead (`Options.WorksDB`, a read-only meta.sqlite - the same stand-in the community repo's own `scripts/key-check.sh` reads on the PR path) through the ONE `resolveWorkKey`, which is also where a root that HOLDS works answers - from the catalogue AND, where the works map has no answer, from that load's own `data/redirects.json`. The two branches are deliberately SYMMETRIC: a submitter pasting a work-page URL for a retired slug is 301'd by metaserve and never learns the slug changed, so the reference they type is the tombstoned one on either repository, and answering "not found" while holding the table that names the survivor would be the bot refusing to read its own data. Three verdicts, deliberately the key check's own with one divergence: live composes, unknown is REFUSED naming the slug (never compose a key that would dangle - the release build stops on one, and the CC BY-SA layer is the most expensive data here) as **needs-human**, since the artifact is stale in ONE direction BY DESIGN and "nothing holds this slug" may be the bot's gap rather than the submitter's mistake - the same verdict class a works-holding root gives for the same state, because one state should not change class with the repository it was sent to - and a RETIRED slug composes under the SURVIVOR with a note, because the bot is choosing the key rather than repairing a written one and writing the tombstoned spelling would open a pull request its own CI rejects. Resolving is not enough on its own: `retiredKeysFor` asks which slugs were retired ONTO the survivor and the sidecar path probes the member at every one of them, because the state a core merge actually leaves is the community entry still keyed by the OLD slug until the re-key sweep lands in the other repository - and a probe under the survivor alone sees nothing and composes a COMPETING member of the same kind for the same book, which the release build then refuses and a human has to unpick. Asked that way round it covers both directions (the form named the retired slug, or the live one). No artifact and no works family is needs-human, never a compose: the bot's inputs are wrong, not the submitter's. The artifact is opened through a URI-ENCODED `file:` DSN (`internal/sqlitedsn.ReadOnly`, which internal/serve's snapshot loader now shares), not a splice - a `?` in an operator-supplied path otherwise ends the file name, silently dropping `mode=ro` and opening (and creating) a different file, which is pinned from the failing side. The ATTACHMENT fetcher (fetch.go) is capped PER TEMPLATE, the cap passed by the caller (`Fetcher` takes it): 25 MiB for the import form's library export (GitHub's own ceiling for a non-image issue attachment, cited in the file) and 1 MiB for a community sidecar - an OpenAudible export of a few hundred books is past 1 MiB, which is what the single sidecar-sized cap refused - and the request deadline is derived from the same cap (`attachmentTimeout`: 20s plus 2s per MiB), so a sidecar fetch does not wait a 25 MiB export's minute on a dead link. It applies its https-plus-GitHub-host rule to the URL the SUBMISSION names and `ghhost.HopPolicy` - https, never an IP literal - to every redirect hop after it, so an allowlisted host answering 302 cannot send the workflow's client (on a runner with network reach the contributor does not have) at the runner's own loopback or a metadata service. The policy goes on a shallow COPY of the caller's http.Client: assigning it through the pointer reconfigured a client the caller still holds and raced any concurrent fetch through it. The verdict's WIRE form is BOUNDED (`Result.MarshalJSON` over wirecap.go): `messages` carries at most 50 lines, 40KB in total and 2KB each (cut at a rune boundary), then one "... and N more" line (unless only one message is left over and showing it costs no more than that line), and the complete list rides beside it as `all_messages` - the intake workflow renders `messages` verbatim into a pull-request body or an issue comment, both of which GitHub stops at 65,536 characters, and prints result.json whole to the run log. In Go the Result keeps every message; the bound keeps the HEAD, so producers order the actionable line first
...
internal/testpack   test support (imported only by _test.go files): `Snapshot` (every file under a dir, keyed by its relative path - the before/after tree comparison the issueform, format and migrate suites share; pkg/pack's in-package tests cannot import it, and the importer's copy also records modtimes), and seeds and reads a pack-layout tree addressed by the old per-record path syntax (a fixture syntax it parses itself), resolved onto family + entry key; shared by the importer, issueform and metaimport suites. fixtures.go additionally RENDERS the minimal schema-valid records a fixture tree is seeded with (WorkJSON/RecJSON/PersonJSON/SeriesJSON/CharactersJSON/RecapsJSON plus their option funcs), shared by internal/audit's detector suite and internal/repair's writer suite - the two had each grown a copy and the copies had drifted on the two details that matter (which separator a "<work>@<position>" member is cut at, and whether a recording's globally-unique ASIN is derived from its id alone or from its work too, which handed two recordings of one slug the same identifier). A fixture that differs between the suite that PROPOSES a repair and the suite that APPLIES it is drift neither suite can catch
...
data/               the CC0 half of the database, in the PACK-SPEC.md range-packed layout: works/ (composites), people/, series/, plus the one non-pack file redirects.json (the slug tombstone table). The CC BY-SA works-community/ family lives in audiosilo-meta-community since the cutover, and a copy of it REAPPEARING here is a red metacheck (the `core` profile makes it an unrecognized location). It also holds `data/go.mod`, a code-free nested module whose only job is to keep the data tree out of this module's zip so pkg/* importers (audiosilo-sidecars) can `go get` a release, pinned by `TestDataTreeIsANestedModule` (module_test.go)
Dockerfile          image: site build + metaserve, NO baked data (the catalogue is fetched from the newest data release at boot; a UI change never rebuilds the database). All three base images are pinned by DIGEST with the tag kept for readability (dependabot's docker entry is what keeps them current), and there is deliberately NO HEALTHCHECK: /healthz is a READINESS probe (it answers 503 while the server waits out a GitHub outage - a healthy process with no data yet) and a Docker healthcheck's "unhealthy" is what autoheal/Swarm/compose supervisors RESTART on, which would turn that patient wait into the crash loop the degraded boot exists to avoid. The orchestrator wires /healthz as readiness/startup itself (audiosilo-docs docs-developers/meta/api.md says so explicitly)
scripts/            operator scripts: the libex dump-to-rows SQL flow (+ README), genrepaths/ (the Go generator that DERIVES audiblegenres.json's marketplace-keyed by_path table and its testdata/genrepaths.json verification file from libex's per-marketplace /categories taxonomies - see the README), and pack-union-merge.sh - the mechanical resolution for a pack-file conflict (a three-way merge of the entries, then metafmt --write), usable by hand on a conflicted file AND as the git merge driver .gitattributes routes data/**/*.json to, with packmerge_test.go driving both through real rebases including every case it must REFUSE (a non-pack file among them: handed data/redirects.json it used to empty the tombstone table and report success) and the add/add shape git's own line merge duplicates
...
.github/            scripts/ai-verify.sh no longer reads a DIFF. ai-verify.yml builds its context.txt by compiling `cmd/metadiff` in the job and rendering the tranche as an ENTRY-LEVEL SUMMARY, with the raw diff of `data/redirects.json` appended (the one non-pack file, small enough to send verbatim); a metadiff failure falls back to the old raw-diff context and says so BOTH in a note at the top of the file and in a `CONTEXT_FORMAT` environment variable the script selects its system prompt by - a prompt explaining how to read an entry-level summary is WRONG, not merely unhelpful, in front of a unified diff, and it is what would tell a model it is reading the whole change when it is reading a truncated part of one. The fallback covers the BUILD too (`go run`, inside the `if`: under `set -e` an unguarded `go build` would abort the step before the fallback it exists for), and if the raw diff is empty as well the step goes RED rather than buying a verdict on a context.txt holding only the note - a data pull request is verified or loudly skipped, never quietly passed. That is the fix for a silent defect, not a tidy-up: the tree is range-packed, so a 100-work sync pull request (#2278) was 136 files and +46,300/-22,193 lines, of which the script's `MAX_INPUT_BYTES` head -c let the model see a tenth - an arbitrary tenth, with nothing in the input saying so. `MAX_INPUT_BYTES` stays as the OUTER bound; metadiff's own `--max-bytes` is what now decides what is dropped, and it drops whole entries with a count. The PROMPT is written to that input (what `+`, `-`, `~` and `...` lines mean, that moved-only entries are omitted, that an absent field reads `(none)`) and the JSON verdict contract is unchanged. It is core-only: three families in the storage-layout paragraph, every record must be CC0-1.0, and a `community` entry or a CC-BY-SA-4.0 record appearing here is ITSELF a finding (the sidecar length/spoiler/position rules went to the community repo's reviewer, which sees diffs that can actually contain one). Issue forms - the four CORE ones (add-work/add-recording/correct-data/import-library); add-characters.yml and add-recaps.yml moved to the community repo with the layer they contribute to, and ISSUE_TEMPLATE/config.yml carries a contact link pointing a sidecar contributor there. The COMPOSER for both stays in internal/issueform (the community repo runs this very cmd/metaissue under the community profile), so TemplateFromLabels still routes characters/recaps - and intake.yml passes `--profile core`, which is what makes a sidecar label that still arrives here (a maintainer applying data:recaps by hand, a bookmarked prefill, the sibling contributor tool) REFUSE-AND-REDIRECT through issueform.templateBelongsElsewhere, naming the community repo's issue chooser, instead of composing a works-community entry into a family this root does not hold - which would be a green-looking bot PR putting CC BY-SA content in the CC0 repo, permanently unmergeable since CI is core-profile too. Machine-parseable ids, data:* routing labels; check + release + image + intake + ai-verify workflows. The intake bot's pull-request body lists its Files from `git status -- data/` AFTER the libex fill step - what the pull request actually carries, which the fill can widen past metaissue's own list - capped at 50 with a count, and renders Notes from metaissue's already-bounded `messages`. check.yml and the intake bot pass --profile core; release.yml checks out audiosilo-meta-community beside this tree and composes the artifact with metabuild --community, accepts a `community-data` repository_dispatch as a third trigger, and names both SHAs in the release notes. intake.yml also rebases the bot's own open PRs on every push to main that touches data/ (the packjson merge driver + metafmt + metacheck, per-PR failures contained to a comment; a rebase the driver resolved is re-rendered and amended before the push); the sweep selects its branches by the `bot-intake` LABEL rather than by branch name, so the series-completion sync bot's pull requests - labelled `data`, `bot-intake` AND `bot-sync`, with `sync:needs-human` marking one parked for a maintainer - are swept by it unchanged, and ai-verify.yml already runs on them (same-repo branches touching data/); permissions are per-job and the rebase sweep has its own concurrency group; release.yml is a SHALLOW checkout now that added_at lives in the data. THREE SHELL SCRIPTS under .github/scripts carry what would otherwise be duplicated or untestable YAML. `require-on-main.sh` is the ANCESTRY GUARD both publishing doors share (release.yml's `workflow_dispatch`, image.yml's `v*` tag): it fetches main TREELESS (`--filter=tree:0` - reachability needs commit objects and nothing else, over a shallow checkout or a complete one) and refuses a HEAD that is not an ancestor, with the caller's own remedy sentence as `$1`. `notify-release.sh` is the WEBHOOK SENDER, a script rather than twenty lines of YAML because internal/serve's `TestReleaseNotifyScript*` runs it in `--print` mode and posts what it emits to the real handler through httptest - the envelope broke silently once (a `printf` in single quotes, so every delivery was a 400 the poller papered over) and sender and receiver cannot drift again; that is scripts/packmerge_test.go's precedent. `pr-changes.sh` is check.yml's `changes` job: ONE listing of a pull request's files, two answers - `go` (the Go gate: build/vet/`go test -race`/golangci-lint/govulncheck, skipped when every changed file is `data/**` (bar `data/go.mod`) or `*.md`, because most pull requests here are the bots' and a tranche of pack files cannot change what `go vet` says) and `compose` (the narrower release path set). Both FAIL OPEN, which the inline compose probe it replaced did not, and both are consumed as STEP-level `if`s over a job output rather than a `paths:` filter - these are required checks and a filtered-out job never reports at all. metacheck/metafmt are never gated: they are the data's own check. image.yml publishes the server image on a `v*` tag: the ancestry guard above (a `v*` matches any commit, and this pushes names people deploy from), `latest` only for a tag with no prerelease hyphen, and buildx provenance + SBOM attestations. Every `uses:` in every workflow is pinned to a COMMIT SHA with the version in a trailing comment - a tag is a moving pointer into somebody else's repository - and .github/dependabot.yml (gomod, npm over site/, github-actions, docker, minor+patch grouped per ecosystem) is what keeps those and the Dockerfile's digests from going stale. Every `setup-go` step reads `go-version-file: .go-version` - a one-line file at the repo root holding the bare MINOR (`1.26`) - together with `check-latest: true`, which is what actually resolves it to the newest 1.26.x patch. It cannot be `go-version-file: go.mod`: go.mod's `go 1.26.0` is an exact patch (`go mod tidy` will not let it stay a bare minor), setup-go takes it as the exact constraint, and `check-latest` then finds no newer patch of it to check for - every job would build on EXACTLY 1.26.0 and every stdlib advisory fixed since that patch would read as ours. setup-go uses a non-go.mod version file's trimmed contents as the version spec verbatim (`parseGoVersionFile`, read in the source AND the shipped dist/ of the pinned v5.6.0, where action.yml still documents only go.mod/go.work; upstream main documents `.go-version` outright), so the file is exactly the old inline `go-version: "1.26"` held in one place instead of eight. A Go bump is therefore FOUR edits and no more: go.mod's `go` line, `.go-version`, the Dockerfile's `golang:<minor>-alpine` tag + digest, and check.yml's govulncheck pin. go.mod carries NO `toolchain` line and its floor is the `.0` patch on purpose: pkg/* is imported by audiosilo-sidecars, whose `go` line must be at least ours, so a patch floor would push a security choice onto every consumer, and a `toolchain` line is read by neither CI (check-latest runs a newer patch) nor the image (the golang base image sets `GOTOOLCHAIN=local`). Its one effect is LOCAL: nothing moves a checkout onto a newer patch - a `GOTOOLCHAIN=auto` checkout on an older minor downloads exactly `go1.26.0`, and an installed 1.26.x below the newest is used as it is - so run a local govulncheck under the newest patch (`GOTOOLCHAIN=go1.26.N`), or it reports stdlib advisories CI does not have. The `vuln` job runs `govulncheck` directly (`go run golang.org/x/vuln/cmd/govulncheck@vX ./...`, pinned) rather than through `golang/govulncheck-action`, which installs `@latest` - see check.yml's comment for why, and bump the pin with `.go-version` - the linter version is PINNED rather than `latest` (a new release adds checks, and a baseline that goes red on a day nobody touched Go code is a broken gate), and workflows.yml runs actionlint (with the shellcheck and pyflakes its image bundles) over `.github/workflows/**` plus shellcheck (`-S style`, its own digest-pinned image) over `.github/scripts/**`, which actionlint cannot reach - it lints a `run:` block and never follows a `bash .github/scripts/x.sh` into the file, so the four scripts that decide what is skipped and what is published had no lint at all
```
...
public so there is no auth, and responses carry permissive CORS. EVERY 500 carries
one fixed body (`internalErrMsg`, written by the one `Server.fail`) and the error's
own text goes to the LOG instead: an internal error names a SQL statement, a path
on the cache volume or a driver message, and every route here is public and
CORS-open, so reflecting it hands that to anyone who can provoke it. The 4xx
messages are untouched - those are about the request, which the caller sent. The
log line QUOTES the method and the path: `r.URL.Path` is the DECODED path, so a
`%0A` in it used to split the entry and let the caller write a log line of their
own. The current
...
file. EVERY artifact-sized writer is additionally BOUNDED, and the bound is a
CEILING whose failure mode must never be being too LOW: a bound the real artifact
exceeds is not a caught attack, it is every refresh failing forever - the whole
asset re-downloaded each poll, maxBytes+1 bytes written to the cache volume each
time - while the server serves stale data. So `decompressBound` is the MAXIMUM of
three arms: 16x the size the release metadata declares for the asset (the real
artifact's gzip ratio measures ~3.8x), a 4 GiB floor, and TWICE THE ARTIFACT
CURRENTLY LOADED, which is the arm that keeps the bound tracking a growing
catalogue instead of needing a constant raised by hand (`Server.expansionBound`
reads the loaded artifact; a release that declares NO size logs a warning naming
the asset and lands on the other two arms - the 1 GiB floor it used to land on was
below the ~1.6 GB artifact it was bounding). A bound is needed at all because a
gzip stream and a zstd frame both declare no output size and the sha256 gate only
fires once the bytes are on disk; hitting it fails the copy, and the temp file
goes exactly as it does on a digest mismatch. The PATCH download takes the
declared size as an EXACT bound instead (`downloadTo`): nothing is decompressed
there, so there is no expansion to allow for. Asset downloads are also
HOST-ALLOWLISTED before the request is built (`checkAssetTarget` over
internal/ghhost: github.com, api.github.com, `*.githubusercontent.com`, over https
AND WITH NO EXPLICIT PORT - the allowlist reads `u.Hostname()`, which strips the
port, so an allowlisted NAME with a port of the release JSON's choosing was
dialled with the bearer token attached). The URLs come out of the release JSON and
`get` attaches this server's token to whatever host they name, so a release naming
another host would otherwise be handed the credential. That is production policy
EXACTLY: a test points the client at an httptest server through an explicit
`ghClient.allowOrigins` (filled by `allowOrigin`, matching a WHOLE origin -
scheme, host and port), not through a clause inside the rule.
A REDIRECT HOP is judged by a DIFFERENT and deliberately weaker rule
(`ghhost.HopPolicy`: https, never an IP literal, chain bounded). GitHub picks the
CDN a `browser_download_url` 302s to and has moved it before (*.s3.amazonaws.com),
so a hop allowlist would one day mean every refresh failing forever - and it would
buy nothing, since net/http strips the Authorization header on a cross-host hop.
Only the ~80-byte checksum assets are read into memory (`downloadSmall`,
...
notification after every asset is uploaded AND read back verified, so the
receiver cannot race an incomplete or corrupt release. The body is composed with
`jq -cn` and the HMAC is taken over exactly those bytes - the `printf` that
preceded it sat in single quotes, so its backslashes were LITERAL and every
delivery was rejected 400 by `webhook.go` with nothing but the poller to notice.
Delivery is now VERIFIED: the step reports the HTTP status, which is what tells a
rejected payload apart from an unreachable receiver. The request body is only a
...
whose terms are all stopwords ("the-a") rather than every initialism. `ftsMatch`
is also where a query is BOUNDED - 256 bytes, cut at a rune boundary, and 64
phrases - since one phrase is one posting-list walk and the length of that list
must be the server's to decide rather than a per-keystroke caller's. Both numbers
are ANCHORED on the real tree rather than chosen for looking round, under the rule
that no real name is cut: measured 2026-09-24 over data/, the longest work title +
subtitle is 252 bytes / 44 terms, the longest person name 161 / 26, the longest
...
work cards from the artifact, and are SELECT-THEN-BUILD: a request reads each
watched series' membership without cards (`watchMembersSQL`: work id, position and
`works.added_at`), asks the news rule ITSELF which of them is news - `watchNewsOf`,
the ONE function that turns a release date and an added_at into a state, an id
suffix, a summary and the item's two timestamps, over release dates read from
`cardFactsByWork` and batched across every watched series' members at once, so the
selection cannot disagree with the card the survivor ends up carrying - and builds
full cards for the survivors alone, RENDERING the decision the selection already
made (`buildWatchItem`) rather than asking again. `watchItem` is the two composed,
for a caller holding a whole card. That the cheap pass and the expensive one agree
is structural rather than a prose invariant: there is one rule and it reads the
two dated facts alone. Reading each series WHOLE through
`snapshot.series`, which is what it did, materialized a card, its authors, its
first series membership and its recordings' facts for every member of up to 200
series and then discarded nearly all of it: measured over the 278,582-work
artifact on the 200 biggest series (10,335 memberships), 309-355ms and 25 MiB
allocated became 120-126ms and 14 MiB, for a byte-identical feed (357 items, the
same ids). `TestWatchFeedMatchesTheWholeSeriesSweep` pins that equivalence against
the sweep it replaced, and
`TestWatchItemDecidesOnTheDatedFactsAlone` pins that the item build adds no
condition over the shared rule - over a fixture holding TWO watched series that
share a work (one news, one not, the second requested by a RETIRED slug), which is
what makes the dedupe's "seen on first SIGHT" claim a test rather than an
assertion. The
feeds let a feed reader deduplicate stable item
...
its own door, `makeSeriesRef` and the genre-claim builders where a claim is
built; the libex credit-LIST refusal still reads the escaped spelling (it must
tell an entity's ';' from a separator) while the vocabulary refusals judge the
decoded name, and internal/issueform's `sections.get` decodes an issue-form
value with the same helper (an attachment field is read raw - pasted JSON has
its own escaping, and the importer decodes an import's text itself). Identity rules: a
...
- **initials spellings are one person** (Class B). `Slugify` turns both the dot
  ...
  RESOLVES a credit rather than creating one goes through `resolvePerson`, the
  same decision `getOrCreatePerson` acts on - without which a merge silently
  dropped the role credit it had just resolved.
...
can make two long titles by one author meet on one slug, a walk counts a hit on
a shortened candidate only when the work there carries the walked title whole. A work title is **cleaned of a trailing
...
- **Libex integration (in progress)**: libex (libexdb.com - the domain moved
  ...
  collisions via the browse-tree hierarchy. Book coverage was never the gap:
  CLAIM coverage is 83.1% (4.49M of 5.40M claims, 2,471 of 2,723 nodes, up from
  77.0% / 2,272 when Romance > Contemporary was found unmapped in every
  marketplace - 71k books had lost `contemporary-romance`, issue #2337), and
  what stays unmapped is deliberate: umbrella nodes naming no single genre
  ("Literature & Fiction", "Mystery, Thriller & Suspense", "Genre Fiction") and
  the children's leaves the children's rule suppresses. A third table,
  `by_path`, serves the sources that state a category LADDER of names instead
  ...
  `TestGenrePathsMatchTheirNodes` re-checks every path in it against its node,
  and `TestChildrensPathsAvoidAdultAdviceGenres` runs the children's rule over
  every children's path of every marketplace. An OpenAudible ladder whose NO
  level maps is reported whole, once; its deliberately-unmapped umbrella levels
  are not); and `metaimport libex --enrich`
  ...
  **contributor-role modeling** (work `credits` + the `credit_role` enum +
  ...
  credits too: a personal export states a role whenever a credit name carries a
  qualifier, so `applyToWork`'s attest scope is live behaviour under the tier
  rule (mirror-only work takes it, attested work keeps what it has), not the
  inert guard the first draft described. GENRES are the exception to that
  rule (LICENSING.md rule 5): an OpenAudible export states ONE category ladder
  ...
  **The SERIES-COMPLETION SYNC BOT is the standing consumer of these modes**:
  ...
  holds, imports them with `metaimport libex` (plus `--enrich` over the touched
  series' existing members), and opens ONE self-merging pull request per cycle -
  the one automation GOVERNANCE.md's "Series-completion bot" section lets
  auto-merge, under the bound and the ai-verify gate stated there and not
  restated here. Its code is that repository's, but its TOOLS are ours: it
  builds metaimport, metafmt and metacheck from a PINNED commit of this repo, so
  a CLI flag, a report shape or a selector rule changed here breaks it silently
  until the pin is bumped there - bumping it is part of making such a change.
