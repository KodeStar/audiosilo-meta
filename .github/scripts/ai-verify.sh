...
- License layer: THIS REPOSITORY HOLDS THE CC0 CORE ONLY. Every record (work/recording/person/series) must be CC0-1.0; flag any record on any other license. The CC BY-SA community layer (characters, recaps and descriptions) moved to the separate repository KodeStar/audiosilo-meta-community, so a diff here that touches ANY path under data/works-community/, or that adds a record carrying CC-BY-SA-4.0, is itself a finding to flag - that content does not belong in this repository and CI refuses the tree.
