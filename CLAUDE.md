...
`go-version: "1.26"` + `check-latest: true` pair on every `setup-go` (go.mod's
`go 1.26.0` is a FLOOR, not the toolchain), which steps `changes` gates and why
