...
than a pass/fail. It never writes to `data/`; a repair pass acting on its report is
a separate change.
...
- `metacheck` counts the collisions still in the tree as an ADVISORY class
  (`normalized-duplicate-works`, 3,270 groups when it landed), so a repair wave's
...
data/               the database, in the PACK-SPEC.md range-packed layout: works/ (composites), works-community/ (the CC BY-SA sidecars), people/, series/, plus the one non-pack file redirects.json (the slug tombstone table)
...
301 is transparent to both. Business
...
- **Data quality (audit landed, PREVENTION landed, repairs next)**: `metaaudit`
  ...
  guard and metacheck's advisory census, all three over the one
