...
- **metacheck / metafmt default to `all`.** "A data root" with nothing said must
  keep meaning the whole database - that is what the fixtures and most tests are -
  and CI passes `--profile core` explicitly (check.yml, the intake bot's
  re-render, release.yml's core-side validation). A wrong reading there is a red
  check.
- **metaaudit / metarepair default to `core`.** They are operator tools a human
  points at THIS tree by typing a command, and metarepair DELETES RECORDS. Under
