...
internal/sqlitedsn  the read-only `file:` DSN a SQLite artifact path is opened with (`ReadOnly`), a LEAF with no dependency of its own. Two unrelated packages open an operator-supplied artifact read-only - internal/serve's snapshot loader and internal/issueform's works-artifact stand-in - and a spliced `file:`+path is not a URI: a '?' in the path starts the QUERY (dropping mode=ro, so the handle is read-WRITE and a path naming nothing is CREATED), a '#' truncates and a literal %2F decodes to '/'. The path is made absolute and URI-encoded instead. It moved here from internal/issueform when serve became the second consumer, because a second spelling is a second chance to lose the read-only guarantee silently; both sides' behavioural tests stayed where they were
...
messages are untouched - those are about the request, which the caller sent. The current
...
file. Both decompression paths are additionally BOUNDED
(`decompressBound`: 8x the size the release metadata declares for the asset, or
the base artifact's size on the patch path, never below a 1 GiB floor), because a
gzip stream and a zstd frame both declare no output size and the sha256 gate only
fires once the bytes are on disk - so without it a runaway expansion fills the
cache volume before anything can reject it; hitting the bound fails the copy, and
the temp file goes exactly as it does on a digest mismatch. Asset downloads are
also HOST-ALLOWLISTED before the request is built (`checkAssetURL`:
github.com, api.github.com, `*.githubusercontent.com` over https, plus the
configured API base's own origin, which is how the tests point the client at an
httptest server) - the URLs come out of the release JSON and `get` attaches this
server's token to whatever host they name, so a release naming another host would
otherwise be handed the credential.
...
is also where a query is BOUNDED - 256 bytes, cut at a rune boundary, and 16
phrases - since one phrase is one posting-list walk and the length of that list
must be the server's to decide rather than a per-keystroke caller's. Both bounds
...
`works.added_at`), asks `watchItem` ITSELF which of them is news over a probe card
carrying the two dated facts the decision reads - the release date coming from
`cardFactsByWork`, batched over every watched series' members at once, so the
selection cannot disagree with the card the survivor ends up carrying - and builds
full cards for the survivors alone. Reading each series WHOLE through
...
`TestWatchItemDecidesOnTheDatedFactsAlone` pins the property it rests on. The
