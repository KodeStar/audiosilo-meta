...
- **studio concatenations are split off a name** (Class A) - a fourth step of the
  ...
  `<person, 2+ tokens> for <studio tail>` need no attestation; every other form
  (of/at connectors, a whitespace-delimited dash, parens/brackets/slash/pipe, and
  the bare concatenation) requires the person half to be **independently attested
  as a credit**, and the bare form requires the removed tail to be attested too.
  The attestation universe is the catalogue's person slugs plus a census of the
  batch's credit names, snapshotted before planning so nothing depends on row
  order. Single-word labels (vox/voz/voce/sprecher) are never in the vocabulary
