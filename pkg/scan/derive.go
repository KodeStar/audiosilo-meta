...
var (
	...
	// the metadata-row side and is deliberately NARROWER on two points: it
	// excludes "part", and it REQUIRES the separator this pattern leaves optional.
	// There the marker is stripped to decide two titles name the same work, so a
	// wrong strip ("The Jungle Book 2") attaches a narration to the wrong work.
	// Here the marker is only read to derive a position from a folder name, which
	// the importer re-validates, so both widenings are safe. Keep the two in step
	// on everything else.
