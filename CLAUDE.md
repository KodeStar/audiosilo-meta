...
- **Libex integration (in progress)**: libex (libexdb.com - the domain moved
  ...
  of a node id (OpenAudible's `genre` field, "Romance:Contemporary"): it is
  DERIVED from libex's per-marketplace `/categories` taxonomies, one entry per
  path whose node resolves differently from its leaf name, so a path-stating
  source lands on exactly what a node-stating one does - lookup is node, then
  path, then name); and `metaimport libex --enrich`
  ...
  **contributor-role modeling** (work `credits` + the `credit_role` enum +
  ...
  qualifier (and an OpenAudible export states its genre ladder), so
  `applyToWork`'s attest scope is live behaviour under the tier rule (mirror-only
  work takes it, attested work keeps what it has), not the inert guard the first
  draft described.
