...
main - build, vet, `go test -race`, `golangci-lint run` and `govulncheck ./...`
beside metacheck/metafmt. The last three are recent: the race detector, the
linter and the vulnerability scan were documented here and run by hand, so the
green baseline this file promises rested on somebody remembering. The linter and
the test suite are their own jobs (independent work finishes sooner), the linter
version is PINNED rather than `latest` (a new release adds checks, and a baseline
that goes red on a day nobody touched Go code is a broken gate), and every
`setup-go` in every workflow passes `check-latest: true` - go.mod's `go 1.25.0`
is a floor, and without it setup-go resolves that to EXACTLY 1.25.0, a toolchain
whose stdlib advisories govulncheck would then report as ours. A fifth workflow,
`workflows.yml`, runs actionlint (with the shellcheck and pyflakes its image
bundles) over `.github/workflows/**` on changes to them - the intake sweep, the
release build and ai-verify are substantial shell programs living in YAML. CI
...
GitHub's "latest" can be either kind. Every asset is READ BACK
before the run is over: both checksum files must come down from the release
byte-identical to the ones just computed and the published `meta.sqlite.gz` must
hash to what its own checksum claims, failing the job otherwise - the assets ARE
